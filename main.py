...
import colorCompiler;

# Varibles:
NAME = "Color Compiler + Mcren's String Importer"
VERSION = "1.00"
IMAGE_PATH = "Image.png"
img = Image.open(IMAGE_PATH); # The image
...
def write_strings_to_file(strings, max_length=512):
  with open("Image Data 1.txt", "w", encoding="UTF-8") as file:
...
4. Use default colors from the Rec Room color pallete.\n\nanswer: """));

# Using the color compiler
if option == 1:
  rrPalette = colorCompiler.main(SYMBOLS,  IMAGE_PATH);
...
#inputing colors manually
  ...
  for i in SYMBOLS_LIST:
    try:
      ...
      if(color == ""): break
      rgbString += str(colorCompiler.rgbToHex(eval(color))) + "\n";
      rrPalette[eval(color)] = i;
      count -= 1; # count-- doesn't work for some reason???
  ...
  rrPalette = {};
  for i, ln in enumerate(open("Hex Colors.txt", "r", encoding="UTF-8").readlines()):
    rrPalette[colorCompiler.hexToRGB(ln)] = SYMBOLS_LIST[i];
# use default colors.
else: pass;
