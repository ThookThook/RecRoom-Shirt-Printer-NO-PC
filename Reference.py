...
from PIL import Image  # PIL is the only required import
...
def write_strings_to_file(strings, filename = "Image Data.txt"):
  filename = "Image Data.txt"
  with open(filename, "w", encoding="UTF-8") as file:
      for i in range(0, len(strings), MaxStringLength):
          file.write("\n".join(strings[i:i+MaxStringLength]) + "\n\n")
...
if __name__ == '__main__':
    try:
        ...
        # Specify the filename where you want to save the compiled strings
        output_filename = "Data.txt"
       # Write the compiled strings to the text file
        write_strings_to_file(main(list_size), output_filename)
